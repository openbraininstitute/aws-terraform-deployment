...
resource "aws_vpc_security_group_ingress_rule" "viz_brayns_allow_port_22" {
  security_group_id = aws_security_group.viz_ec2.id
...
resource "aws_autoscaling_group" "viz" {
  name                = "viz_asg"
  max_size            = 2
  min_size            = 0
...
resource "aws_ecs_capacity_provider" "viz" {
  ...
  auto_scaling_group_provider {
    ...
    auto_scaling_group_arn         = aws_autoscaling_group.viz.arn
...
resource "aws_ecs_cluster_capacity_providers" "viz_cluster_cas" {
  cluster_name       = aws_ecs_cluster.viz.name
