...
def default(event, context):
    ...
    if not ip:
        ...
        try:
            with urlopen(f"http://{ip}:8000/health", timeout=1) as response:
    ...
    return {
        "statusCode": 500,
        "body": json.dumps({"message": "Error forwarding message"}),
    }
...
def disconnect(event, context):
    conn_id = event["requestContext"]["connectionId"]
    svc_vlab = event["requestContext"]["authorizer"]["SVC_VLAB"]
    data = DDB.delete_item(
        TableName=DDB_WS_CONN_TASK, Key={"conn": {"S": conn_id}}, ReturnValues="ALL_OLD"
    )
    task_submit_time = datetime.fromisoformat(
        data["Attributes"]["task_submit_time"]["S"]
    )
    ...
    if task_end_time - task_submit_time > timedelta(days=1):
        L.error(
            "Task was running for too long! Submitted: %s, finished: %s, vlab: %s, task: %s",
            task_submit_time,
            task_end_time,
            svc_vlab,
            task,
    ...
    if task_submit_time < end_time_1st_day_of_month:
        ...
        old_month_duration = (
            end_time_1st_day_of_month - task_submit_time
        ).total_seconds()
        ...
        L.info(
            "Task duration old month: %s, new month: %s.",
            old_month_duration,
            new_month_duration,
        )
    else:
