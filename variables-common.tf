...
variable "azure_datasync_agent_activation_key_uswest2" {
  type        = string
  sensitive   = true
  description = "Activation key for the AWS DataSync agent deployed on Azure for the uswest2 AWS region. Short-lived, must be set right before deploy."
}

variable "azure_datasync_agent_activation_key_useast1" {
  type        = string
  sensitive   = true
  description = "Activation key for the AWS DataSync agent deployed on Azure for the useast1 AWS region. Short-lived, must be set right before deploy."
}
...
variable "azure_datasync_agent_vm_resource_group_name" {
  type = string
}
