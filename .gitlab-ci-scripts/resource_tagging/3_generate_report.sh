...
report="$(
    [[ ${num_resources_untagged} -gt 0 ]] && color="ff0000" || color="009900"
    echo -n "{color:#${color}}*Current overall status of '{{${TAG_KEY}}}' tagging* [Updated: ${CI_JOB_STARTED_AT}]:{color}\n\n"
    ...
    if [[ ${num_resources_untagged} -gt 0 ]]; then
        ...
        echo -n "\n\nCurrently, no action is required by any team (i.e., all of the resources are properly identified)."
    fi
)"

# Update the ticket with the latest information and print the summary without the Jira format
curl -s -u obp_bot:${AUTH_TOKEN} -X PUT --data "{\"body\": \"${report}\"}" \
                                 -H "Content-Type: application/json" \
                                 https://bbpteam.epfl.ch/project/issues/rest/api/2/issue/${TICKET_ID}/comment/${COMMENT_ID} 1>/dev/null
echo -e "${report}" | tr -d '*' | sed -r "s|'\{\{([^\}]+)\}\}'|'\1'|g" | sed -r "s|\{co[^\}]*}||g"
