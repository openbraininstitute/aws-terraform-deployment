    ...
    ml_producer_pmc = [
      {
        ...
        input = jsonencode({
          containerOverrides = [
            {
              name    = "ml_producer",
              command = ["pu-producer", "pmc-oa-opendata", module.ml_sqs.queue_url, "--start-date", "01-01-1500", "--index", "pmc_paragraphs_v2", "--parser-name", "jats_xml", "--prefix", "oa_comm/xml/all", "oa_noncomm/xml/all", "author_manuscript/xml/all", "--file-extension", "xml", "-v"]
            }
          ]
        })
