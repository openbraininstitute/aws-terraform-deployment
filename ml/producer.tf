    ...
    ml_producer_pmc = [
      {
        ...
        input = jsonencode({
          containerOverrides = [
            {
              name    = "ml_producer",
              command = ["pu-producer", "pmc-oa-opendata", module.ml_sqs.queue_url, "--index", "pmc_paragraphs_v3", "--parser-name", "parse/jats_xml", "--prefix", "oa_comm/xml/all", "oa_noncomm/xml/all", "author_manuscript/xml/all", "--file-extension", "xml", "-v"]
            }
          ]
        })
    ...
    ml_producer_local = [
      {
        ...
        input = jsonencode({
          containerOverrides = [
            {
              name    = "ml_producer",
              command = ["pu_producer", "ml-paper-bucket", module.ml_sqs.queue_url, "--index", "pmc_paragraphs_v3", "--sign-request", "-v"]
            }
          ]
        })
