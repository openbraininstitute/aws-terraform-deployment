...
# Blazegraph needs some storage for its configuration
resource "aws_efs_file_system" "blazegraph_config" {
  #ts:skip=AC_AWS_0097
  creation_token         = "${var.blazegraph_efs_name}_config"
  availability_zone_name = "${data.aws_region.current.name}a"
  encrypted              = false #tfsec:ignore:aws-efs-enable-at-rest-encryption

  tags = {
    Name = "${var.blazegraph_efs_name}_config"
  }
}
